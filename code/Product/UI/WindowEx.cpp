...
bool WindowEx::showMaxBox(){
	return m_showMaxBox;
}

void WindowEx::setShowMaxBox(bool showMaxBox){
	m_showMaxBox = showMaxBox;
	if (m_maxOrRestoreButton){
        m_maxOrRestoreButton->setVisible(showMaxBox);
    }
}

bool WindowEx::showMinBox(){
	return m_showMinBox;
}

void WindowEx::setShowMinBox(bool showMinBox){
	m_showMinBox = showMinBox;
	if (m_minButton){
        m_minButton->setVisible(showMinBox);
    }
}
...
void WindowEx::getProperty(const String& name, String *value, String *type){
    if (name == L"showmaxbox"){
        *type = L"bool";
		*value = FCStr::convertBoolToStr(showMaxBox());
    }
    else if (name == L"showminbox"){
        *type = L"bool";
		*value = FCStr::convertBoolToStr(showMinBox());
    }
	else if (name == L"windowstate"){
        *type = L"enum:WindowStateA";
        WindowStateA windowState = getWindowState();
        if (windowState == WindowStateA_Max){
            *value = L"Max";
        }
        else if (windowState == WindowStateA_Min){
            *value = L"Min";
        }
        else if (windowState == WindowStateA_Normal){
            *value = L"Normal";
        }
    }
    else{
		FCWindow::getProperty(name, value, type);
    }
}

ArrayList<String> WindowEx::getPropertyNames(){
	ArrayList<String> propertyNames = FCWindow::getPropertyNames();
	propertyNames.add(L"ShowMaxBox");
	propertyNames.add(L"ShowMinBox");
	propertyNames.add(L"WindowState");
	return propertyNames;
}

void WindowEx::maxWindow(){
...
void WindowEx::onAdd(){
    ...
    if (!m_maxOrRestoreButton){
		...
		m_maxOrRestoreButton->setVisible(m_showMaxBox);
		m_maxOrRestoreButton->addEvent(&clickButton, FCEventID::CLICK, this);
    }
    if (!m_minButton){
		...
		m_minButton->setVisible(m_showMinBox);
		m_minButton->addEvent(&clickButton, FCEventID::CLICK, this);
    }
}
...
void WindowEx::setProperty(const String& name, const String& value){
    if (name == L"showmaxbox"){
		setShowMaxBox(FCStr::convertStrToBool(value));
    }
    else if (name == L"showminbox"){
		setShowMinBox(FCStr::convertStrToBool(value));
    }
	else if (name == L"windowstate"){
		String str = FCStr::toLower(value);
        if (str == L"max"){
			setWindowState(WindowStateA_Max);
        }
        else if (str == L"min"){
			setWindowState(WindowStateA_Min);
        }
        else if (str == L"normal"){
			setWindowState(WindowStateA_Normal);
        }
    }
    else{
		FCWindow::setProperty(name, value);
    }
}
...
void WindowEx::update(){
    ...
    if (m_maxOrRestoreButton && m_maxOrRestoreButton->isVisible()){
		FCPoint location ={width - 48, 2};
        m_maxOrRestoreButton->setLocation(location);
    }
    if (m_minButton && m_minButton->isVisible()){
