...
extern "C"
{
#include "..\\third_party\\7zC\\7z.h"
#include "..\\third_party\\7zC\\7zAlloc.h"
#include "..\\third_party\\7zC\\7zCrc.h"
#include "..\\third_party\\7zC\\7zFile.h"
#include "..\\third_party\\7zC\\7zVersion.h"
};
...
namespace FaceCat
{
	...
	#ifdef _WIN32
			{
					...
					return SZ_ERROR_MEM;
				{
					...
					int numChars = WideCharToMultiByte(fileMode ?
						...
						0, (LPCWSTR)s, len, (char *)buf->data, size, &defaultChar, &defUsed);   // debug
	...
	int Decode7z::Decode(const wchar_t* dest,const wchar_t* src)
	{
		if (!dest || dest[0]==0 || !src || src[0]==0)
		{
			//param error
			return -1;
		}
		...
		wcscpy(pPath,dest);
		wcscat(pPath,L"\\");
		UInt16 destPath[MAX_PATH] = {0};
		UInt16 dir2create[MAX_PATH] = {0};
		...
		if (InFile_OpenW(&archiveStream.file, src))
		{
				...
				*/
				...
				for (i = 0; i < db.db.NumFiles; i++)
				{
					...
					if (!testCommand)
					{
						...
						wcscpy((WCHAR*)destPath,pPath);
						wcscat((WCHAR*)destPath,(WCHAR*)name);
						for (j = 0; name[j] != 0; j++)
							if (name[j] == '/')
							{
								if (fullPaths)
								{
									name[j] = 0;
									wcscpy((WCHAR*)dir2create,pPath);
									wcscat((WCHAR*)dir2create,(WCHAR*)name);
									nret = MyCreateDir(dir2create);
								...
								else
								{
									//destPath = name + j + 1;
									wcscpy((WCHAR*)destPath,pPath);
									wcscat((WCHAR*)destPath,(WCHAR*)(name + j + 1));
								}
							}

							if (f->IsDir)
							{
								nret = MyCreateDir(destPath);
								//printf("\n");
								continue;
							}
